...
def line_writer(list_type: ListType) -> LineWriter:
    """TODO."""
    return AutoLineWriter.from_list_type(list_type, "https://example.com/logos/")
...
def test_get_default_lines(line_writer: LineWriter, streams: list[Stream]) -> None:
    """TODO."""
    expected = [
        (
    ...
    actual = [line_writer.get_lines(stream) for stream in streams]

    assert actual == expected


@pytest.mark.parametrize("list_type", [ListType.KODI])
def test_get_kodi_lines(line_writer: LineWriter, streams: list[Stream]) -> None:
    """TODO."""
    expected = [
        (
...
def test_get_pipe_lines(line_writer: LineWriter, streams: list[Stream]) -> None:
    """TODO."""
    expected = [
        (
            '#EXTINF:-1 tvg-name="Das Erste HD" tvg-id="daserste.de" group-title="IPTV-DE" tvg-logo="https://example.com/logos/tv/daserste.png",Das Erste HD',  # noqa: E501
